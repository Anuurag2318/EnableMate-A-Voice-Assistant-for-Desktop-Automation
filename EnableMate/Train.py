...
for intent in intents['intents']:
    ...
    for pattern in intent['patterns']:
        w=nlp_processor.tokenize(pattern)
        all_words.extend(w)
        xy.append((w,tag))    
    
ignore_words=['?','!','.',',']
all_words=[nlp_processor.stem(w) for w in all_words if w not in ignore_words]
...
for (pattern_sentence,tag) in xy:
    bag=nlp_processor.bag_of_words(pattern_sentence,all_words)
    x_train.append(bag)    
